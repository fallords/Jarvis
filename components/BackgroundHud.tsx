...
export default function BackgroundHud() {
  ...
  useEffect(() => {
    try {
      // give a narrow shape for the connection object to avoid `any` lint errors
  ...
  const localALevel = aLevelRef.current;
      ...
      for (let i = 0; i < orbitCount; i++) {
        ...
        ctx.beginPath();
        ctx.arc(lx, ly, lowPower ? 2 : 4 + (i % 3) * 1.5, 0, Math.PI * 2);
  ...
  const burstCount = isMobile ? (lowPower ? 4 : 6) : lowPower ? 6 : 10;
      for (let i = 0; i < burstCount; i++) {
        ...
        ctx.beginPath();
        ctx.arc(bx, by, lowPower ? 1.5 : 2 + (i % 3) * 0.5, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(188,250,255,0.9)';
        ctx.fill();
      }
    ...
    const onResize = () => {
      ...
      devicePixelRatioRef.current = dpr2;
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (dt >= 33) {
        ...
        for (let i = 0; i < bands; i++)
          out[i] = Math.min(1, out[i] + Math.random() * 0.02 + lvl * 0.04);
        ...
        if (frameCounter % effectiveThrottle === 0) {
          ...
          setSpectral((prev) => {
            let changed = false;
            for (let i = 0; i < bands; i++) {
              if (Math.abs((prev[i] ?? 0) - out[i]) > 0.003) {
                changed = true;
                break;
              }
            }
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0">
      ...
      <motion.div
        ...
        style={{
          transform: `translate3d(${(mousePos.x - 0.5) * (lowPower ? 6 : 18)}px, ${(mousePos.y - 0.5) * (lowPower ? 4 : 12)}px, 0)`,
        }}
      >
        <svg
          width="640"
          ...
          style={{ filter: lowPower ? 'blur(2px)' : undefined }}
        >
          <circle
            cx="320"
            cy="320"
      ...
      <motion.div
        ...
        style={{
          transform: `translate3d(${(mousePos.x - 0.5) * -10}px, ${(mousePos.y - 0.5) * -6}px, 0)`,
        }}
      >
        ...
        {[
          ...
          return (
            <motion.path
              ...
              animate={{
                strokeDashoffset: [0, -300, 0],
                opacity: [0.05 + aLevel * 0.05, 0.22 + aLevel * 0.4, 0.05 + aLevel * 0.05],
              }}
              transition={{
                repeat: Infinity,
                duration: a.dur - Math.min(4, aLevel * 3),
                ease: 'linear',
              }}
            />
          );
      ...
      <motion.div
        ...
        initial={{ scale: 1 }}
        animate={{
          scale: [1, 1.02 + aLevel * 0.6, 1],
          opacity: [0.5 + aLevel * 0.2, 0.95, 0.5 + aLevel * 0.2],
        }}
        transition={{ repeat: Infinity, duration: 3.6, ease: 'easeInOut' }}
        style={{
          transform: `translate3d(${(mousePos.x - 0.5) * 6}px, ${(mousePos.y - 0.5) * 4}px, 0)`,
        }}
      >
        <svg width="520" height="520" viewBox="0 0 520 520" fill="none" className="opacity-60">
          <circle
            ...
            stroke="#34d399"
            strokeWidth="2"
            strokeOpacity={0.06 + aLevel * 0.12}
          />
        </svg>
      </motion.div>
        ...
        >
          <div
            style={{
              ...
              borderRadius: 9999,
              background:
                'radial-gradient(circle at 40% 40%, rgba(56,189,248,0.09), rgba(14,165,233,0.02) 35%, transparent 60%)',
              filter: 'blur(28px)',
              opacity: 0.9,
            }}
          />
          <div
            style={{
              ...
              borderRadius: 9999,
              background:
                'radial-gradient(circle at 50% 50%, rgba(125,211,252,0.26), rgba(56,189,248,0.06) 45%, transparent 70%)',
              boxShadow: `0 0 ${18 + aLevel * 60}px rgba(56,189,248,${0.12 + aLevel * 0.6})`,
            }}
          />
          ...
          <motion.div
            style={{
              ...
              height: 88,
              borderRadius: 9999,
              background: 'radial-gradient(circle, #e6fffb 0%, #7dd3fc 45%, #0369a1 100%)',
              boxShadow: `0 0 ${12 + aLevel * 48}px rgba(14,165,233,${0.5 + aLevel * 0.5})`,
              border: '1px solid rgba(255,255,255,0.08)',
            }}
            animate={{
              scale: [1, 1 + aLevel * 0.28, 1],
              rotate: [0, 18 * aLevel, 0],
              opacity: [0.95, 1, 0.95],
            }}
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <motion.div
              ...
              style={{
                ...
                height: 60 + i * 60,
                borderRadius: 9999,
                border: `1.2px solid rgba(125,211,252,${0.06 + i * 0.03 + aLevel * 0.08})`,
              }}
              animate={{
                scale: [0.6 + i * 0.12, 1 + aLevel * 0.18 + i * 0.06],
              ...
              return (
                <g key={`orbit-${i}`}>
                  ...
                  <line
                    ...
                    x2={round(
                      320 +
                        Math.cos(((i + 1) / (lowPower ? 4 : 6)) * Math.PI * 2) *
                          (lowPower ? 160 : 220)
                    )}
                    y2={round(
                      320 +
                        Math.sin(((i + 1) / (lowPower ? 4 : 6)) * Math.PI * 2) *
                          (lowPower ? 160 : 220)
                    )}
                    stroke="#60a5fa"
      ...
      >
        <div
          style={{
            ...
            borderRadius: 9999,
            background:
              'linear-gradient(90deg, rgba(0,0,0,0) 0%, rgba(56,189,248,0.22) 50%, rgba(0,0,0,0) 100%)',
            filter: 'blur(18px)',
            ...
            <React.Fragment key={i}>
              <motion.div
                ...
                style={{
                  ...
                  left: `${left}px`,
                  top: `${top}px`,
                  boxShadow: `0 0 ${8 + aLevel * 20}px rgba(34,211,238,${0.5 + aLevel * 0.5})`,
                }}
                animate={{ scale: [1, 1 + 0.8 * (0.6 + aLevel), 1], opacity: [0.7, 1, 0.7] }}
                transition={{
                  repeat: Infinity,
                  duration: 2.6 + (i % 4) * 0.4 - aLevel,
                  delay: i * 0.05,
                }}
              />
      ...
      <motion.div
        className="absolute top-6 left-6 rounded-md bg-[rgba(5,30,45,0.28)] p-3"
      ...
      >
        <div
          className="mb-2 h-2 w-40 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500"
          style={{ transform: `scaleX(${0.9 + aLevel * 0.4})` }}
        />
        <div className="flex gap-2">
          <div
            className="h-3 w-24 rounded bg-[rgba(255,255,255,0.06)]"
            style={{ opacity: 0.6 + aLevel * 0.35 }}
          />
          <div
            className="h-3 w-8 rounded bg-[rgba(255,255,255,0.03)]"
            style={{ opacity: 0.45 + aLevel * 0.25 }}
          />
        </div>
      </motion.div>

      <motion.div
        className="absolute top-10 right-6 rounded-md bg-[rgba(5,30,45,0.22)] p-3"
        initial={{ x: 20, opacity: 0 }}
      ...
      >
        <div
          className="mb-2 h-2 w-36 rounded-full bg-gradient-to-r from-sky-400 to-cyan-500"
          style={{ transform: `scaleX(${0.92 + aLevel * 0.38})` }}
        />
        <div className="flex gap-2">
          <div
            className="h-3 w-20 rounded bg-[rgba(255,255,255,0.05)]"
            style={{ opacity: 0.6 + aLevel * 0.3 }}
          />
          <div
            className="h-3 w-10 rounded bg-[rgba(255,255,255,0.03)]"
            style={{ opacity: 0.45 + aLevel * 0.25 }}
          />
        </div>
      </motion.div>
      ...
      {/* bottom waveform */}
      <div
        className="pointer-events-none absolute right-0 bottom-6 left-0 flex items-center justify-center"
        style={{ transform: `translateY(${-aLevel * 6}px)` }}
      >
        <motion.svg width="640" height="80" viewBox="0 0 640 80" fill="none" className="opacity-90">
          ...
          <motion.path
            ...
            transition={{
              repeat: Infinity,
              duration: 3.6 - Math.min(1.6, aLevel * 1.8),
              ease: 'linear',
            }}
          />
              ...
              {spectral.map((v, i) => {
                ...
                return (
                  <rect
                    ...
                    height={h}
                    rx={2}
                    fill={`rgba(125,211,252,${0.12 + v * 0.7})`}
                  />
                );
              })}
            </g>
      ...
      <motion.div
        ...
        style={{
          background:
            'radial-gradient(circle at 30% 20%, rgba(10,120,160,0.06), transparent 20%), radial-gradient(circle at 70% 80%, rgba(0,80,120,0.05), transparent 30%)',
        }}
      />
